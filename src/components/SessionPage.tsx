...
import SessionTrack from './util/SessionTrack'
import {
  Button,
  Listbox,
  ListboxButton,
...
export default function SessionPage () {
  ...
  const {
    data: raceData,
    isFetched: raceFetched,
    isError: raceDataError
  } = useQuery<DetailedSession>({
    queryKey: [`sessions/detailed/${session_key}`],
    queryFn: () =>
      baseApi.get(`/sessions/detailed/${session_key}`).then(res => res.data),
  ...
  const {
    data: drivers,
    isFetched: driversFetched,
    isError: driversDataError
  } = useQuery<Driver[]>({
  ...
  const {
    data: driversLapData,
    isFetched: lapsFetched,
    isError: lapsDataError
  } = useQuery<DetailedLapData[]>({
    queryKey: [`/laps/detailedForAllDrivers/${session_key}`],
    queryFn: () =>
      baseApi
        .get(`/laps/detailedForAllDrivers/session_key=${session_key}`)
        .then(res => res.data),
    refetchOnWindowFocus: false,
    staleTime: 1000 * 60 * 5
  })
  ...
  const handleLapSelect = (lap_number: number) => {
    selectedDrivers.forEach(driver_number => {
      const lapData = driversLapData?.filter(() =>
        selectedDrivers.includes(driver_number)
      )
      if (lapData === undefined) {
        return
      }
      const driverLapData = lapData.find(
        data => data.driverNumber === driver_number
      )
  ...
  const activeCircuit = useMemo(() => {
    const circuitLongName = raceData?.circuitShortName
    if (circuitLongName != undefined) {
      const circuitFinalName: string = circuitLongName
        .split('-')[0]
        .toLowerCase()
        .trim()
        .replace(' ', '_')
        .split('circuit')[0]
      return circuitFinalName
    } else {
      return undefined
    }
  }, [raceData])

  if (driversFetched && lapsFetched && raceFetched) {
    if (lapsDataError || raceDataError || driversDataError) {
      return <ErrorDisplay msg='Session not found!' />
    } else {
      return (
        <div className='bg-dark-black p-2 rounded-2xl mt-5 w-full'>
          <div className='flex flex-row p-2 m-2 items-center'>
            <img className='w-25 p-2' src={raceData?.countryFlag} />
            <h3 className='lg:text-2xl text-lg'>
              {raceData?.meetingOfficialName}
            </h3>
          </div>
          <div className='flex flex-row w-full'>
            {drivers != null && (
              <div className='flex flex-col w-full'>
                <div className='rounded-2xl m-2 p-2 flex flex-row overflow-x-auto'>
                  {selectedDrivers?.map(driver_id => (
                    <div
                      className='m-2 min-w-40 rounded-lg bg-blue-800 p-0.5 group grid'
                      key={driver_id}
                    >
                      <p className='col-start-1 row-start-1 m-1.5 transition-opacity duration-200 group-hover:opacity-0 pointer-events-none text-center'>
                        {
                          drivers.find(d => d.driverNumber == driver_id)
                            ?.fullName
                        }
                      </p>
                      <button
                        onClick={() => handleDriverRemove(driver_id)}
                        className='col-start-1 row-start-1 opacity-0 group-hover:opacity-100 transition-opacity duration-200 cursor-pointer flex justify-center items-center text-red-500'
                      >
                        <UserRoundX />
                      </button>
                    </div>
                  ...
                  <div className='flex'>
                    {selectedDrivers.length !== 5 && (
                      <select
                        onChange={e =>
                          handleDriverAddition(Number(e.target.value))
                        }
                        value=''
                      >
                        ...
                        {drivers
                          ...
                          .map(driver => {
                            if (driversLapData == undefined)
                              return (
                                <option key={driver.driverNumber}>
                                  No Data
                                </option>
                              )
                            const driverData = driversLapData.find(
                              d =>
                            ...
                            return (
                              <option
                                ...
                                disabled={!hasData}
                                className={
                                  !hasData ? 'text-gray-400' : 'text-black'
                                }
                              >
                                {driver.fullName}
                              </option>
                ...
                <div className='flex flex-row ms-6'>
                  <Button
                    onClick={handleLapSelectPrev}
                    className='bg-slate-900 border border-slate-800 px-2 py-2 rounded-lg'
                  >
                    <ChevronLeft />
                  </Button>
                  <div className='relative w-26 ms-1 me-1'>
                    <Listbox value={selectedLap} onChange={handleLapSelect}>
                      ...
                      <ListboxOptions
                        as='div'
                        className='max-h-60 absolute overflow-y-auto opacity-80 rounded-lg border border-slate-800 bg-slate-900 p-1 shadow-xl [--anchor-gap:4px]'
          ...
          {driversLapData != undefined && (
            <div className='flex'>
              {driversLapData.filter(lapData =>
                selectedDrivers.includes(lapData.driverNumber)
              ) != undefined && (
                <SessionTrack
                  circuitName={activeCircuit}
                  lapData={driversLapData.filter(lapData =>
                    selectedDrivers.includes(lapData.driverNumber)
                  )}
