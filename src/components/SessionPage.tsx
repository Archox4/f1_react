...
import type { Driver, DetailedLapData, DetailedSession } from "../interfaces/types";
...
export default function SessionPage(){
    ...
    const {data: raceData, isFetched: raceFetched, isError: raceDataError} = useQuery<DetailedSession>({
        queryKey: [`sessions/detailed/${session_key}`],
        queryFn: () => baseApi.get(`/sessions/detailed/${session_key}`).then(res => res.data),
    ...
    const {data: drivers, isFetched: driversFetched, isError: driversDataError} = useQuery<Driver[]>({
    ...
    const {data: driversLapData, isFetched: lapsFetched, isError: lapsDataError} = useQuery<DetailedLapData[]>({
    ...
    const handleLapSelect = (lap_number: number) =>{
        selectedDrivers.forEach((driver_number) => {
            const lapData = driversLapData?.filter(() => selectedDrivers.includes(driver_number));
    ...
    const activeCircuit = useMemo(() => {
        ...
        if(circuitLongName != undefined){
            const circuitFinalName: string = circuitLongName.split('-')[0].toLowerCase().trim().replace(' ','_').split("circuit")[0];
            console.log(circuitFinalName);
        ...
        if(lapsDataError || raceDataError || driversDataError){
            ...
            <div className="bg-dark-black p-2 rounded-2xl mt-5 w-full">
                <div className="flex flex-row p-2 m-2 items-center">
                    ...
                    <h3 className="text-2xl">{raceData?.meetingOfficialName}</h3>
                </div>
            ...
            {drivers != null && (
                        ...
                        {selectedDrivers.length !== 5 && (
                            <select onChange={(e) => handleDriverAddition(Number(e.target.value))} value="">
                                ...
                                {drivers?.filter((driver) => driver && !selectedDrivers.includes(driver.driverNumber)).map((driver) => {
                                    if(driversLapData == undefined) return (<option key={driver.driverNumber}>No Data</option>)
                ...
                {driversLapData != undefined && (
                    ...
                    {driversLapData.filter((lapData) => selectedDrivers.includes(lapData.driverNumber)) != undefined &&
                        <SessionTrack circuitName={activeCircuit} lapData={driversLapData.filter((lapData) => selectedDrivers.includes(lapData.driverNumber))} selectedLap={selectedLap}/>
                    }
                    </div>
                )}
        </div>
